...
var TerminalShell = {
    commands: {
        help: function help(terminal) {
            ...
            keys = Object.keys(this.commands),
            keys.sort();

            $.each(keys, function(id, name) {
...
var Terminal = {
    ...
    init: function() {
        ...
        $(document)
            ...
            // Autocomplete commands / filenaems if they are the only ones
            .bind('keydown', 'tab', ifActive(function(e) {
                e.preventDefault();
                if (Terminal.buffer.split(' ')[0] != "sudo"){
                    var found = false;
                    var replaceWith = "";
                    if (Terminal.buffer.indexOf(" ") != -1){
                        var filePart = Terminal.buffer.substr(Terminal.buffer.indexOf(" ") + 1);
                        if (filePart != ""){
                            for (var file_name in TerminalShell.pwd) {
                                if (file_name.indexOf(filePart) == 0){
                                    if (found == true){
                                        found = false;
                                        break;
                                    }
                                    found = true;
                                    replaceWith = file_name;
                                }
                            }
                            if (found){
                                Terminal.buffer = Terminal.buffer.replace(new RegExp(filePart + '$'), replaceWith);
                                Terminal.setPos(Terminal.buffer.length);
                                Terminal.updateInputDisplay();
                            }
                        }
                    }else{
                        var cmdPart = Terminal.buffer;
                        if (cmdPart != ""){
                            for (var cmd_name in TerminalShell.commands) {
                                if (cmd_name.indexOf(cmdPart) == 0){
                                    if (found == true){
                                        found = false;
                                        break;
                                    }
                                    found = true;
                                    replaceWith = cmd_name;
                                }
                            }
                            if (found){
                                Terminal.buffer = Terminal.buffer.replace(new RegExp(cmdPart + '$'), replaceWith);
                                Terminal.setPos(Terminal.buffer.length);
                                Terminal.updateInputDisplay();
                            }
                        }
            ...
            .keyup(function(e) {
                ...
                if (keyName in {'ctrl':true}) {
                    Terminal.sticky.set(keyName, false);
                }else if (!(keyName in {'left':true, 'right':true, 'up':true, 'down':true, 'scroll':true})) {
            ...
            .keydown(function(e) {
                var keyName = $.hotkeys.specialKeys[e.which];
                if (keyName in {'ctrl':true}) {
                    Terminal.sticky.set(keyName, true);
                }else if(keyName in {'scroll':true}){
                    Terminal.sticky.toggle(keyName);
                }/*else if (!(keyName in {'left':true, 'right':true, 'up':true, 'down':true})) {
                    Terminal.sticky.resetAll();
                }*/
            })
            .bind('keydown', 'ctrl+w', $.proxy(ifActive(function(e) {
                e.preventDefault();
                this.deleteWord();
            }), this))
            .bind('keydown', 'ctrl+h', $.proxy(ifActive(function(e) {
                e.preventDefault();
                Terminal.deleteCharacter(false);
            }), this))
            .bind('keydown', 'ctrl+l', $.proxy(ifActive(function(e) {
                e.preventDefault();
                this.clear();
            }), this))
            .bind('keydown', 'ctrl+a', $.proxy(ifActive(function(e) {
                e.preventDefault();
                this.setPos(0);
            }), this))
            .bind('keydown', 'ctrl+e', $.proxy(ifActive(function(e) {
                e.preventDefault();
                this.setPos(this.buffer.length);
            }), this))
            .bind('keydown', 'ctrl+d', $.proxy(ifActive(function(e) {
                e.preventDefault();
                this.runCommand('logout');
            }), this))
            .bind('keydown', 'ctrl+c', $.proxy(ifActive(function(e) {
                if ($('.browser')){
                    e.preventDefault();
                    $('.browser').last().remove();
                }
            ...
            .keypress($.proxy(ifActive(function(e) {
                ...
                if (!this.sticky.keys.ctrl && character) {
                    this.addCharacter(character);
                    e.preventDefault();
                }
            }), this));
        $('#screen').height($('body').outerHeight() - $('#welcome').height() - $('#credit').height() - $('#bottomline').height() - 4);
        ...
        $('#screen').hide().fadeIn('norm', function() {
    ...
    processInputBuffer: function(cmd) {
        ...
        if (this.output) {
            if (cmd == "Yes" || cmd == "yes" || cmd == "Y" || cmd == "y"){
            ...
            }else if (cmd == "No" || cmd == "no" || cmd == "N" || cmd == "n"){
                if (this.noExec){
                    var tmp_exec = this.noExec;
                    delete this.yesExec;
                    delete this.noExec;
                    tmp_exec(this);
                    this.promptShow();
                ...
                if (this.noExec){
                    ...
                    if (this.resumeGuide){
                        ...
                        var tmp_exec = this.resumeGuide;
                        delete this.resumeGuide;
                        tmp_exec(this);
                    }
                }
            }
    ...
    promptShow: function() {
        this.config.prompt = 'guest@xyendev:' + this.current_path + '$ ';
        this.updateInputDisplay();
    },
    ...
    runCommand: function(text) {
        ...
        var interval = window.setInterval($.proxy(function typeCharacter() {
            ...
            if (index < text.length) {
                if (!Terminal.iOS){
                    var keySnd = Terminal.keypress_snd.cloneNode(true);
                    keySnd.volume = 0.1;
                    keySnd.play();
                }
...
$(document).ready(function() {
    ...
    Terminal.keypress_snd = new Audio();
    Terminal.keypress_snd.src = "static/key.mp3";
    
    Terminal.iOS = /(iPad|iPhone|iPod)/g.test( navigator.userAgent );
    if (Terminal.iOS){
        Terminal.init();
    }else{
        Terminal.keypress_snd.addEventListener('canplaythrough',  function(e) { Terminal.init(); }, false);
    }
